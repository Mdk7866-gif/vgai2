  ...
  return (
    <div ref={ref} className="relative">
      ...
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="w-full flex items-center justify-between px-3.5 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900/60 text-left cursor-pointer transition-all focus:outline-none focus:ring-2 focus:ring-brand-500/50"
      >
        <span className="flex items-center gap-2 min-w-0">
          {selected.badge && (
            <span className="px-1.5 py-0.5 rounded-md text-[10px] font-bold bg-brand-600 text-white flex-shrink-0">
              {selected.badge}
            </span>
          )}
                  ...
                  {o.badge && (
                    <span
                      className={`px-1.5 py-0.5 rounded-md text-[10px] font-bold flex-shrink-0 ${
                        isSelected ? "bg-brand-600 text-white" : "bg-slate-200 dark:bg-slate-700 text-slate-500 dark:text-slate-300"
                      }`}
                    >
                      {o.badge}
                    </span>
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-2">
        ...
        <span className="text-xs font-semibold text-white bg-brand-600 px-2 py-0.5 rounded-md tabular-nums">
