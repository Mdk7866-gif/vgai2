...
The shared `default_style_templates.demo_image_urls` column stores up to three display-ordered examples for each starter style. The Starter Templates popup shows the primary frame plus gallery thumbnails; clicking either uses the shared zoom viewer. Catalog editing and one-call, three-variation AI generation happen only in `vgai2admin`. On import, only image #1 is copied into the user's own `style_templates.demo_image_url`; the extra two examples stay catalog-only, keeping the library card compact and independently owned.
