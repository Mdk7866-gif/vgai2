...
- **Current state**: `<user_id>/characters/` and `<user_id>/<project_id>/{scene_images,scene_animation,thumbnail_image}/` are wired up. `project_characters/` and `voiceovers/` are not — `project_characters.snapshot_character_sheet_url` deliberately reuses the character's existing `characters/` URL rather than uploading a per-project copy (so it's never touched by project delete/cleanup, which is scoped to the `<project_id>/` prefix and can't reach `characters/`), and voiceover generation is still a stub.
