...
- **Current state**: `<user_id>/characters/` and `<user_id>/<project_id>/{scene_images,scene_animation,thumbnail_image}/` are wired up. `<user_id>/<project_id>/project_characters/` is now wired up too, but only for a project-scoped character-snapshot edit's *replacement* image — `project_characters.snapshot_character_sheet_url` still starts out as a reused copy of the character's existing `characters/` URL on import (so it's untouched by project delete/cleanup at that point, which is scoped to the `<project_id>/` prefix and can't reach `characters/`), and only gets a real per-project upload once that snapshot's image has actually been edited for this project. `voiceovers/` is not wired up — voiceover generation is still a stub.
